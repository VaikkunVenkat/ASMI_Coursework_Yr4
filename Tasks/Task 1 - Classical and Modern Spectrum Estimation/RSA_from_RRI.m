...
figure(1);
plot(t1,xRRI1_Vaikkun,'b','LineWidth',1);xlabel('Time [s]');ylabel('RRI [mV]');title('RRI of Trial 1 (Normal)'); grid on; grid minor; legend('Normal'); set(gca,'FontSize',18)
figure(2);
plot(t2,xRRI2_Vaikkun,'r','LineWidth',1);xlabel('Time [s]');ylabel('RRI [mV]');title('RRI of Trial 2 (Fast)'); grid on; grid minor; legend('Fast'); set(gca,'FontSize',18)
figure(3);
plot(t3,xRRI3_Vaikkun,'g','LineWidth',1);xlabel('Time [s]');ylabel('RRI [mV]');title('RRI of Trial 3 (Slow)'); grid on; grid minor; legend('Slow'); set(gca,'FontSize',18)
...
for i = 1: numRRI
    ...
    legend({'Original Periodogram','AR(2)','AR(5)','AR(8)','AR(11)'},'FontSize',14)
end
...
uconf = 0.2;
lconf = -uconf; hold on;
plot([1 15],[1 1]'*[lconf uconf],'k--');grid on; grid minor;
